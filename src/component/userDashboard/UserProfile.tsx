    ...
    <div className="p-6 bg-white rounded-lg shadow-md max-w-3xl mx-auto">
      <div className="flex items-center gap-6 mb-6">
        <div className="relative">
          ...
          />
          <label htmlFor="profile-upload" className="absolute bottom-0 right-0 bg-purple-600 p-2 rounded-full cursor-pointer hover:bg-purple-700 transition">
            <FaCamera className="text-white" />
          <input type="file" id="profile-up  load" accept="image/*" className="hidden" onChange={handleFileUpload} />
          </label>
          {uploadProgress > 0 && uploadProgress < 100 && (
            <div className="absolute top-full mt-1 text-xs text-gray-700">{uploadProgress}%</div>
                ...
                <div>
          <h2 className="text-2xl font-bold text-purple-700">{profile.firstname} {profile.lastname}</h2>
          <p className="text-gray-500">{profile.email}</p>
        </div>

        <div className="ml-auto">
          <button type="button" onClick={() => setIsModalOpen(true)} className="btn btn-outline btn-sm text-purple-700 hover:bg-purple-100 flex items-center gap-2">
            <Edit2 size={16} /> Edit
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-800">
        <div><p className="font-semibold">Phone</p><p>{profile.contactPhone || 'N/A'}</p></div>
        <div><p className="font-semibold">Role</p><p className="capitalize">{profile.role}</p></div>
        <div><p className="font-semibold">Address</p><p>{profile.address || 'N/A'}</p></div>
        <div><p className="font-semibold">Joined</p><p>{new Date(profile.createdAt ?? '').toLocaleDateString()}</p></div>
      </div>

      {isModalOpen && (
        <div className="modal modal-open bg-black bg-opacity-50">
          <div className="modal-box relative max-w-lg">
            <button type="button" onClick={() => setIsModalOpen(false)} className="btn btn-sm btn-circle absolute right-2 top-2" title="Close">
              <X size={12} />
            </button>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div>
                ...
                <textarea {...register('address')} className="textarea textarea-bordered w-full" rows={3} />
              </div>
              <div className="modal-action">
                <button type="button" onClick={() => setIsModalOpen(false)} className="btn btn-outline">Cancel</button>
                <button type="submit" className="btn bg-purple-600 hover:bg-purple-700 text-white" disabled={isUpdating}>
